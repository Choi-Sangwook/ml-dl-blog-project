...
> 반면 **NumPy로 만든 1D 합성곱 데모**(§5)는 추가 설치가 필요 없어 **글쓴이가 직접 실행한 실제 출력**입니다.
> 또한 강의의 RNN 코드는 `torchtext.legacy` 등 **현재 버전에서 제거·폐기된 API**를 사용합니다. 해당 부분은 **강의 원문 그대로 인용**하되, 지금 그대로는 실행되지 않는다는 점을 분명히 표시했습니다.
...
## 2. 왜 자연어 처리에 딥러닝이 필요한가 🟦

사람의 언어는 **같은 글자라도 상황에 따라 의미가 달라집니다.**

> 🟦 (강의 예시)
> - "**배**가 아프다" → 배 = 신체
> - "**배**를 먹었다" → 배 = 과일

컴퓨터는 글자만 보고는 이 둘을 구분하기 어렵습니다. 그래서 딥러닝 모델은 단어 하나만 보는 것이 아니라

- **문맥(Context)**
- **주변 단어**
- **문장의 구조**

를 함께 학습해 의미를 추론합니다. 과거에는 사람이 직접 규칙을 만들어 프로그램을 짰지만, 지금은 대부분 **대량의 문장을 학습해 스스로 언어의 특징을 배우는** 딥러닝 방식이 쓰입니다.
...
- **현재 자연어 처리의 핵심 모델.** RNN을 쓰지 않고 **Attention만** 사용.
- 🟦 구조: `입력 → Embedding → Positional Encoding → Multi-Head Attention → Feed Forward → 출력`
- **장점**: 매우 빠르고 **병렬 처리**가 가능하며 긴 문장도 잘 처리.
- 🟩 **보충**: RNN은 단어를 **앞에서부터 하나씩** 처리해 병렬화가 어렵지만, Transformer는 **문장 전체를 한 번에** 보기 때문에 학습이 빠릅니다. 대신 단어 순서 정보를 잃지 않으려고 **Positional Encoding**으로 위치를 따로 알려 줍니다.
...
> 🟦 **실제 활용 사례**: 감성 분석(리뷰 긍·부정), 기계 번역, 챗봇·AI 비서, 문서 요약, 검색 시스템, 음성 비서, 코드 생성 보조 등. 현재 대부분의 최신 서비스는 **Transformer 계열**(BERT·GPT)을 기반으로 동작합니다.

> 🟩 **주의(과장 금지)**: "Transformer가 항상 최고", "BERT가 모든 문제에서 우월" 같은 단정은 피하세요. 데이터 크기·과제·자원에 따라 **작은 CNN/RNN이 더 적합**할 때도 많습니다. 비교표의 ○/✗는 **경향**이지 절대 규칙이 아닙니다.
...
3. **완전 연결층(Fully Connected Layer)**: 압축된 특징으로 **최종 분류**
...
for i in range(len(words) - 2 + 1):     # stride=1
    ...
    score = np.sum(window * kernel)     # 원소별 곱 후 전부 더함
    feature_map.append(round(float(score), 2))
...
2. 영문/문자 데이터를 숫자로 변환
3. `data.info()`, 결측값 확인, **중복 제거**(`drop_duplicates()`)
...
> 🟩 **중요(데이터 누수 방지)**: `Tokenizer`(단어 사전)와 빈도 통계는 **반드시 훈련 데이터에만 `fit`** 해야 합니다. 평가 데이터까지 포함해 사전을 만들면 **테스트 정보가 학습에 새어 드는(data leakage)** 문제가 생깁니다. 강의 순서(5번 분리 → 6번 토크나이저)가 이 점에서 올바릅니다.
...
model = Sequential([
    ...
    GlobalMaxPooling1D(),
    Dense(1, activation="sigmoid"),         # 이진 분류(스팸 O/X)
])
...
> 🟩 **주의(평가)**: 스팸 분류처럼 **클래스가 불균형**(정상 80% : 스팸 20%)할 때는 **정확도(accuracy)만으로 판단하면 안 됩니다.** "전부 정상"이라고만 찍어도 정확도 80%가 나오기 때문입니다. **precision·recall·F1** 을 함께 봐야 합니다. (강의는 "정확도가 높음"이라고만 언급하므로, 이 부분은 보충이 필요합니다.)
...
- `torchtext.legacy`(`Field`, `BucketIterator`, `IMDB.splits`)는 **torchtext 0.12(2022) 이후 제거**되었습니다. 게다가 torchtext 라이브러리 자체가 현재 **유지보수 종료(개발 중단)** 상태입니다.
- `pl.metrics.Accuracy`는 **`torchmetrics`** 패키지로 분리되었습니다.
- `pl.Trainer(gpus=1)`의 `gpus=` 인자도 폐기되어, 최신 버전은 `accelerator="gpu", devices=1` 형태를 씁니다.

> 그래서 아래 코드는 **강의 학습 흐름을 이해하기 위한 "원문 인용"** 으로만 제시합니다(🟦). **지금 그대로는 실행되지 않으며**, 실제로 돌리려면 최신 PyTorch 방식(`torch.utils.data.Dataset`/`DataLoader`, Hugging Face `datasets`/`tokenizers` 등)으로 옮겨야 합니다. 이 글에서는 **거짓 출력 대신 구조 설명**에 집중합니다.
...
설치: `pip install pytorch-lightning torchtext`

### 6.4 LSTM 모델 구조(개념) 🟦

강의는 `LightningModule`을 상속해 LSTM 모델을 정의합니다. **부분 코드(원문에 `...`로 생략된 부분 포함)** 이므로 **구조 이해용**으로만 보세요.
...
> 🟩 **shape 읽는 법(입문자용)**: `permute(1, 0, 2)`는 차원의 **순서를 바꾸는** 연산입니다. 여기서는 `(배치, 문장길이, 임베딩차원)`을 `(문장길이, 배치, 임베딩차원)`으로 바꿉니다. 옛 PyTorch LSTM 기본 설정이 **"길이(시간축)가 맨 앞"** 인 `(seq_len, batch, feature)` 형식을 기대했기 때문입니다. (최신 코드는 `batch_first=True` 옵션을 자주 씁니다.)
>
> 🟩 **손실함수 짝 확인**: 출력이 **클래스 점수(logits)** 이고 손실이 `nn.CrossEntropyLoss()`이면 **모델 마지막에 softmax를 넣지 않습니다.** `CrossEntropyLoss`가 내부에서 softmax 계산을 함께 처리하기 때문입니다(중복 적용 금지).
...
3. **불균형 데이터에서 정확도만 보기**: 스팸 20% 문제에서 정확도 80%는 의미가 약함. → precision·recall·F1.
4. **softmax 이중 적용**: `CrossEntropyLoss` 앞에 softmax를 또 넣기. → 손실함수가 내부 처리하므로 빼기.
5. **GPU 가정**: `device='cuda'`로 고정. → `torch.cuda.is_available()`로 확인 후 결정.
6. **폐기된 API를 그대로 실행**: `torchtext.legacy`는 현재 동작하지 않음. → 최신 데이터 파이프라인으로 이전.
...
- 🟩 PyTorch 공식 문서 — `torch.nn.CrossEntropyLoss`, `torch.nn.LSTM`
- 🟩 PyTorch Lightning 공식 문서 — `LightningModule`, `Trainer`
- 🟩 Keras 공식 문서 — `Conv1D`, `GlobalMaxPooling1D`, `Embedding`
- 🟩 torchtext 마이그레이션 안내(legacy 제거 관련) — 공식 GitHub/문서 확인 권장
