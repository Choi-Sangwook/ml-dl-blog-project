...
> 📝 DAY1에서 DNN~Transformer 모델 지도를, DAY2에서 텍스트 분류를 봤습니다. 이번 DAY3에서는 현대 NLP를 이해할 때 반드시 거치는 **대표적인 사전훈련 언어 모델 BERT**를 다룹니다. 핵심은 **"이미 대규모 텍스트로 학습된 모델을 내려받아, 내 문제에 맞게 추가 학습(파인튜닝)한다"** 는 것입니다. 영화평 감성(이진), 뉴스 카테고리(다중), IMDB 감성 분류를 **Hugging Face Transformers**로 실습합니다.

> 💡 **이 글의 표기 약속**
> - 🟦 **(강의 PDF)** : 강의 자료에 직접 나온 개념·코드·출력
> - 🟩 **(보충/수정)** : 입문자를 위해 글쓴이가 덧붙인 설명, 또는 PDF 코드를 실행 가능하게 고친 수정본
>
> ⚠️ **코드 실행 안내**: 이 강의는 **코드 중심**이지만, 작성 환경에 **PyTorch·transformers가 설치되어 있지 않아** 실습 코드를 **직접 실행하지 않았습니다.** PDF에 이미 나온 출력(데이터 shape, `value_counts` 등)은 🟦로 인용하며, **출력값을 새로 지어내지 않았습니다.** 토큰화 개념 데모(§6.3)와 `train_test_split` 반환값 확인만 순수 파이썬으로 **직접 실행한 실제 결과**입니다.
> 또한 PDF 코드 일부에 **버전·논리 불일치**가 있어(예: `train_test_split` 반환값 개수, BERT-Base인데 `layer.23` 사용, IMDB shape), PDF 원문은 🟦로 **짧게 인용**하고 실행 가능한 형태는 🟩 **수정본**으로 구분해 제시합니다.
...
> 🟩 **핵심 직관**: BERT는 "언어의 일반 상식"을 먼저 대량으로 익힌 모델입니다. 그래서 **대부분의 입문 실습에서는 처음부터 사전훈련하지 않고**, 이 모델을 가져와 **내 작은 데이터로 조금만 더 학습**하면 됩니다. 강의의 비유처럼 *"영어를 잘하는 사람이 비슷한 다른 언어를 빨리 배우는 것"* 과 같습니다.
...
| 전략 | BERT 본체에서 학습하는 부분 | 설명 |
|---|---|---|
| 전략 1 | (없음) 모든 BERT 층 Freeze | 분류기 head만 학습 |
...
```python
# 🟦 PDF 원문 — BERT-Large(24층) 기준 "layer.23"
for name, param in model.bert.named_parameters():
    if (not name.startswith("pooler")) and ("layer.23" not in name):
        param.requires_grad = False
```

> 🟩 **중요(버전·모델 불일치 수정)**: 위 `"layer.23"`은 **BERT-Large(24층, 인덱스 0~23)의 마지막 층**을 가리킵니다. 그런데 §6에서 쓰는 한국어 모델 `kykim/bert-kor-base`는 **BERT-Base(12층, 인덱스 0~11)** 입니다(공식 config의 `num_hidden_layers=12`). Base에는 `layer.23`이 없으므로 이 코드를 그대로 쓰면 **마지막 층까지 전부 동결**되어 의도와 달라집니다. **Base라면 `"layer.11"`** 로 바꿔야 "마지막 Encoder 층만 학습"이 됩니다.

> 🟩 **참고**: 위 코드는 `model.bert`(BERT 본체)의 파라미터만 다룹니다. 분류용 head(`model.classifier`)는 새로 붙는 층이라 **기본적으로 학습 대상**입니다. 즉 전략 1도 "아무것도 학습 안 함"이 아니라 **분류기 head는 학습**합니다.

> 🟩 **왜 동결하나?**: 데이터가 적을 때 BERT 전체를 학습하면 과적합되거나 학습이 불안정할 수 있습니다. 일부만 학습하면 **상대적으로 빠르고 과적합 위험을 줄일 수 있습니다.** 단, "어떤 전략이 항상 최고"는 아니며 데이터 양에 따라 다릅니다.
...
전체 흐름은 **데이터 분리 → Dataset 정의 → 토크나이저·모델 로드 → 학습(Trainer) → 예측** 순서입니다.
...
> 🟦 PDF는 `train_test_split(dataset.index, ...)` 처럼 입력 배열을 **1개만** 넘긴 뒤 `train_idx, test_idx, _, _` 처럼 **4개**로 받습니다.
>
> 🟩 그러나 scikit-learn `train_test_split`은 **입력 배열이 1개이면 결과도 2개**(`train, test`)입니다(반환 길이 = `2 × 입력 배열 수`). 로컬에서 직접 확인했습니다. 아래는 실행 가능한 **수정본**입니다.
...
> 🟩 **데이터 누수 주의(정밀하게)**: BERT의 `from_pretrained` 토크나이저는 **이미 학습된 고정 vocabulary**를 사용하므로, 토크나이저를 불러오는 것 자체가 내 데이터에 `fit`되는 전처리는 아닙니다(이 점에서 일반적인 누수와는 다릅니다). 그래도 **Dataset 객체 생성과 평가는 Train/Validation/Test를 먼저 나눈 뒤** 각각 만들고, **Test는 모델 선택·하이퍼파라미터 조정에 쓰지 않고 마지막 평가에만** 사용합니다. (만약 새 vocabulary나 전처리 통계를 직접 학습한다면 반드시 Train에만 `fit` 해야 합니다.)
...
> 🟩 아래는 PDF가 글로 설명한 Dataset 구조를 **실행 가능하게 옮긴 수정본**입니다. 라벨은 분류용 정수이므로 **`dtype=torch.long`** 으로 명시합니다.

```python
# 🟩 실행 가능한 Dataset (PDF 설명 기반 수정본)
import torch
...
class BertDataset(torch.utils.data.Dataset):
    def __init__(self, reviews, labels, tokenizer, max_len=512):  # BERT 최대 길이 512
...
# 이진 분류: num_labels 기본값은 2
model = BertForSequenceClassification.from_pretrained(bert_model_name)
```

> 🟩 **출력층·손실 이해**: `BertForSequenceClassification`은 BERT 위에 **분류기(Linear) head**를 붙인 모델입니다. 이진 분류면 출력 logits의 shape는 `(batch_size, 2)`이고, `labels`를 함께 넣으면 모델이 **교차 엔트로피(Cross Entropy) 손실을 자동으로 계산**해 반환합니다. 그래서 우리가 직접 softmax나 손실함수를 붙일 필요가 없습니다.

🟦 미세조정 전략 적용(§4의 전략 3, **Base이므로 `layer.11`**):

```python
# 🟩 BERT-Base(12층) 기준 수정: 마지막 Encoder 층은 layer.11
for name, param in model.bert.named_parameters():
...
> 🟩 **실행 순서 주의(수정)**: `Trainer`에 넘기는 `compute_metrics`와 `output_dir`에 쓰는 `model_dir`는 **먼저 정의**해야 합니다. 그렇지 않으면 `NameError`가 납니다. 또 `compute_metrics`는 PDF에 정의가 없어 직접 만듭니다.

```python
# 🟩 실행 가능한 수정본: 먼저 model_dir와 compute_metrics 정의
import numpy as np
...
training_args = TrainingArguments(
    ...
    weight_decay=0.01,
    save_strategy="epoch",
    eval_strategy="steps",   # ⚠️ 구버전 transformers에서는 evaluation_strategy
    eval_steps=500,
)
...
> 🟩 **버전 주의(택 1)**: 평가 전략 인자는 `transformers` 버전에 따라 이름이 다릅니다. **현재 Hugging Face 문서 기준은 `eval_strategy`**, 구버전은 `evaluation_strategy` 입니다. 설치 버전을 `import transformers; print(transformers.__version__)` 로 확인한 뒤 **버전에 맞는 하나만** 사용하세요.
...
🟦 데이터 출처(PDF): **공공데이터포털 · 한국언론진흥재단 뉴스빅데이터 메타데이터(올림픽, 2021)** (https://www.data.go.kr). 🟩 직접 찾으려면 검색어 `한국언론진흥재단 뉴스빅데이터 메타데이터 올림픽 2021` 로 검색하면 됩니다.
...
> 🟩 다중 분류는 출력층이 **클래스 수(8)만큼의 logits** 이 되고, 손실은 내부적으로 **교차 엔트로피**가 쓰입니다(`BertForSequenceClassification`이 자동 처리). Dataset 클래스는 `review/sentiment` 대신 `news/category`를 쓰도록만 바꾸면 됩니다.

🟩 다중 분류 평가지표는 클래스별 균형을 보는 **macro 평균**을 권장합니다.

```python
# 🟩 보충: 다중 분류용 compute_metrics
...
> 🟩 이름의 `query/key/value`는 Self-Attention의 핵심 3요소입니다(DAY1 Attention 참고). `encoder.layer.0~11`이 **12층**이라는 점에서 이 모델이 **BERT-Base** 임을 확인할 수 있습니다 → 동결 전략에서 마지막 층은 `layer.11`. `print(name, param)` 으로 출력하면 각 층의 **Weight/Bias 텐서값과 shape**까지 확인할 수 있습니다(학습 가능한 파라미터 점검 용도).

> 🟩 **중요(IMDB 라벨 처리)**: 흔히 쓰는 `IMDB Dataset.csv`는 `review`, `sentiment` 두 열이고 `sentiment`가 **문자열**(`positive`/`negative`)입니다. 문자열을 그대로 `torch.tensor(...)`에 넣으면 실패하므로, **0/1 정수로 매핑**한 뒤 사용합니다. 분류이므로 `stratify`도 넣습니다.

```python
# 🟩 IMDB 라벨 매핑 + stratify 적용 (실행 가능한 수정본)
import pandas as pd
from sklearn.model_selection import train_test_split
...
train_idx, test_idx = train_test_split(
    dataset.index, test_size=0.2, stratify=dataset.label, random_state=42)
train_set = dataset.iloc[train_idx].reset_index(drop=True)
test_set  = dataset.iloc[test_idx].reset_index(drop=True)

tr_idx, val_idx = train_test_split(
    train_set.index, test_size=0.3, stratify=train_set.label, random_state=42)
valid_set = train_set.iloc[val_idx].reset_index(drop=True)
train_set = train_set.iloc[tr_idx].reset_index(drop=True)
```

🟦 토크나이저는 **모델과 호환되는 것**(`bert-base-uncased`)을 쓰며, **Dataset을 만들기 전에 먼저 정의**합니다(영어 모델에는 영어 토크나이저). 데이터 분리·Dataset·Trainer 흐름은 §6과 동일합니다.

```python
# 🟩 (순서 중요) 토크나이저를 Dataset 생성보다 먼저 정의
from transformers import BertTokenizerFast
tokenizer = BertTokenizerFast.from_pretrained("bert-base-uncased")

# train/valid/test Dataset 모두 생성 (label 열 사용)
train_set_dataset = BertDataset(
...
> 🟩 **PDF 출력값 주의(불일치, 추정)**: PDF는 IMDB 절에서도 shape를 `(84000,3)/(36000,3)/(30000,3)`으로 적었는데, 이는 **§6(영화평) 수치를 재사용한 것으로 보입니다.** 실제 **IMDB는 약 5만 행**이고 열도 `review, sentiment` 2개입니다. 직접 실행하면 다른 값이 나옵니다.
...
3. **`train_test_split` 반환값**: 배열 1개를 넘기고 4개로 받기. → 1개 → 2개 반환(`2 × 입력 수`).
4. **문자열 라벨**: IMDB `sentiment`("positive"/"negative")를 그대로 텐서로. → 0/1 매핑 후 `dtype=torch.long`.
5. **정의 순서**: `model_dir`·`compute_metrics`를 `Trainer` 뒤에 정의. → 먼저 정의.
6. **데이터 누수**: 나누기 전에 통계/전처리를 전체에 적용, Test로 모델 선택. → 분리 후 적용, Test는 최종 평가만.
7. **불균형/다중 분류에서 정확도만 보기**: → precision·recall·F1(다중은 macro).
8. **버전 인자**: `evaluation_strategy` vs `eval_strategy`. → 설치 `transformers` 버전 확인 후 택 1.
9. **PDF 출력값 맹신**: IMDB shape 등 일부는 앞 절 수치 재사용. → 직접 실행해 확인.
...
파인튜닝 (Hugging Face Transformers)
  1) 데이터 분리: Train/Test → Train/Valid (stratify, 반환값 2개)
  2) Dataset: __init__/__len__/__getitem__ → input_ids, attention_mask, labels(torch.long)
  3) Tokenizer/Model: from_pretrained (모델·토크나이저 짝 맞추기)
  4) Freeze 전략: requires_grad (Base는 layer.11, head는 기본 학습)
  5) model_dir·compute_metrics 먼저 정의 → TrainingArguments + Trainer
  6) trainer.predict(test) 로 최종 평가

작업별 차이
  - 이진: num_labels 기본(2), average="binary"
  - 다중: num_labels=8, average="macro"
...
- 🟩 Hugging Face Transformers — [`BertForSequenceClassification`](https://huggingface.co/docs/transformers/model_doc/bert#transformers.BertForSequenceClassification), [`Trainer`/`TrainingArguments`](https://huggingface.co/docs/transformers/main_classes/trainer)
- 🟩 한국어 BERT — [`kykim/bert-kor-base`](https://huggingface.co/kykim/bert-kor-base) · 영어 — [`bert-base-uncased`](https://huggingface.co/google-bert/bert-base-uncased)
- 🟩 scikit-learn — [`train_test_split`](https://scikit-learn.org/stable/modules/generated/sklearn.model_selection.train_test_split.html), [`precision_recall_fscore_support`](https://scikit-learn.org/stable/modules/generated/sklearn.metrics.precision_recall_fscore_support.html)
