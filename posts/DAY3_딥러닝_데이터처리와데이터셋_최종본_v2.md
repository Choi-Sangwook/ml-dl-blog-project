...
> - **7장 NumPy 코드의 출력은 실제 실행으로 확인**한 값입니다. **MNIST·CIFAR의 Keras 학습 코드는 직접 실행해 결과를 확인**하세요(자세한 실행 안내는 4.5에). 임의의 결과는 넣지 않았습니다.
...
```python
# (선택) CIFAR-10 샘플을 라벨과 함께 확인 — 컬러 실사진이라 MNIST보다 어렵다는 점을 직접 봄
import matplotlib.pyplot as plt
...
for i in range(8):
    plt.subplot(1, 8, i + 1)
    plt.imshow(X_train[i])                # 컬러(RGB)라 cmap 불필요
    plt.title(class_names[int(y_train[i])]); plt.axis("off")
plt.tight_layout(); plt.show()
```
...
PDF는 행렬 곱을 보는 여러 관점도 소개합니다. 예를 들어 $B=[\,b_1\ b_2\ \cdots\ b_r\,]$ 처럼 **열벡터** $b_j$ 로 나누면 $AB=[\,Ab_1\ Ab_2\ \cdots\ Ab_r\,]$ 로 볼 수 있습니다. 또 $A$의 $k$번째 **열벡터**를 $a_k$, $B$의 $k$번째 **행**을 $r_k^{\mathsf{T}}$(행벡터)라고 하면, 행렬 곱은 다음 **외적(outer product)들의 합**으로도 표현됩니다.
