...
> 🟩 **(보충)** 보통 **은닉층=ReLU 계열, 출력층=문제 유형에 맞게**(이진=Sigmoid, 다중=Softmax, 회귀=활성화 없음)를 씁니다. "어떤 함수가 항상 최선"은 아니며 데이터·문제에 따라 달라집니다. 단, 이는 *개념상* 출력 활성화이며, **PyTorch 학습 코드에서는 손실 함수(`BCEWithLogitsLoss`·`CrossEntropyLoss`)가 시그모이드·소프트맥스를 내부에서 처리**하므로 모델 마지막에 직접 넣지 않습니다(2.5 참고).
...
| **SGD** | 1개씩 | 빠르고 지역최적 탈출에 도움이 될 수 있으나, 진동·불안정 |
