...
- **분류 문제 → 교차 엔트로피(Cross Entropy)**: 모델이 정답 클래스에 부여한 확률을 평가하는 손실
...
딥러닝의 목표는 **예측 오차(Loss)를 최소화하는 가중치**를 찾는 것입니다. 단순한 1차원 볼록 함수에서는 미분값이 0인 지점이 최솟값입니다. 그러나 실제 신경망에서는 기울기가 0에 가까운 지점이 정지점 후보일 뿐이며, 전역 최솟값인지는 보장되지 않습니다. 게다가 실제 모델은 가중치가 **수백만~수천억 개** 입니다(예: 이미지 분류 CNN, 자연어 Transformer, GPT 같은 대규모 언어모델). 이걸 한 번에 정확히 푸는 것은 사실상 불가능합니다.
...
- 기울기가 **음수** → 반대 방향으로 이동
- 기울기가 **0 근처** → 이 1차원 볼록 직관에서는 최솟값에 가까운 상태
...
아래 코드는 PDF의 예제를 현재 TensorFlow 공식 beginner quickstart 방식으로 보완한 것입니다. 🟩
...
> PDF p20의 원래 코드는 출력층에 `softmax`를 넣고 `sparse_categorical_crossentropy`(문자열)를 쓰는 방식입니다. 그 조합도 서로 호환되어 올바르게 동작하지만, 위 코드는 현재 TensorFlow 공식 beginner quickstart 방식을 따른 보충 예제입니다.
...
패딩 없이(`P=0`) stride 1로 합성곱하면, N개 데이터에 크기 K인 커널을 적용한 출력은 **N − K + 1개**로 줄어듭니다. (예: 7개 → 5개) 🟦
