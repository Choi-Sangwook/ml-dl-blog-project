...
즉, **입력층에서 시작 → 은닉층으로 특징 학습 → 출력층에서 결과 생성 → 손실함수·역전파로 가중치를 반복 수정**하며 문제에 적절한 예측 모델을 만드는 전체 과정이 "네트워크 모델 설계"입니다.
...
> - 학습 코드에서 Sigmoid·Softmax를 출력층에 넣지 않는 이유가 이것입니다. Sigmoid·Softmax는 **학습이 끝난 뒤 사람이 확률로 해석할 때** 적용합니다. (Softmax 수식과 확률 해석은 DAY3에서 다뤘습니다.)
...
학습이 끝나면 **평가 모드**로 바꿔 추론합니다. 아래는 **평가 모드 전환과 확률 변환 동작을 확인**하는 부분 코드로, 위 예제의 `model`·`X`를 그대로 재사용합니다. (실제로는 별도의 검증/시험 데이터로 평가해야 합니다.)

```python
# 평가 모드로 전환: Dropout·BatchNorm 동작이 학습 때와 달라짐
model.eval()
