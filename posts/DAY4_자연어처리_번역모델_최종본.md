...
| 통촉하여 주시옵소서. | Please contact me. | Please let me know. | **Please understand my situation.** (또는 **Please have mercy on me.**) | **고어(옛말)** 학습 데이터 부족 |

> 🟦 **핵심 교훈**:
> - 번역기는 **관용 표현(idiom)** 을 모르면 문자 그대로 번역해 의미가 달라집니다.
> - 문맥에 따라 **정답이 하나가 아닐 수도** 있습니다(‘어딜 도망가?’).
> - 최근 신경망 번역기는 **사투리**도 상당량 학습해 자연스럽게 번역합니다.
> - 반면 **고어·오래된 표현**은 데이터가 부족해 품질이 낮아질 수 있습니다. 번역 모델은 단어뿐 아니라 **시대적 표현**도 충분히 학습해야 합니다.
...
> 🟦 **개선 예시(PDF 스크린샷)**: 같은 "돼지 주물럭"도 번역기와 시점에 따라 결과가 달라집니다. 구글 번역은 `Pork loin`처럼 돼지 부위는 잡았지만 ‘주물럭’ 조리 의미를 놓쳤고("돼지 부위는 맞으나 주물럭 부분의 번역을 놓침"), 파파고는 `Marinated Grilled Pork`로 PDF 기준 더 자연스럽게 번역한 예시가 제시됩니다("정확함 / 자연어 데이터를 기반으로 학습 중"). 즉 음식명 번역은 단순 단어 치환보다 **자연어 데이터·문맥 학습**의 영향을 크게 받으며, 최근 신경망 번역기는 과거보다 자연스럽게 번역하는 경우가 많아졌습니다.
...
> 🟩 **보충(발전 시점)**: Google은 **2016년**에 Google Translate에 신경망 번역(NMT)을 적용하기 시작했습니다. 강의는 GNMT를 **RNN 기반 Seq2Seq 인코더-디코더**로 설명합니다(§5에서 자세히 다룸). 공식 GNMT 논문 기준으로는 **LSTM 기반 인코더-디코더에 attention과 residual connection을 포함**한 시스템이었으며, 이후 **Transformer** 계열에서 attention이 더 중심적인 구조로 쓰이게 되었습니다. (DAY4의 중심은 이어서 볼 RNN 기반 Seq2Seq입니다.)
...
> 🟦 GNMT의 성능 향상 항목: 문맥 이해 능력 향상, 번역 정확도 향상, 사람과 유사한 자연스러운 번역, **다양한 언어 지원 확대**.
...
```text
[Encoder LSTM]  How → is → weather → today?   (질문을 순서대로 읽고 의미를 압축)

[Decoder LSTM]  입력 : <go> → It → is → sunny → today
...
🟦 (강의 다이어그램, p.17-p.20) 인코더에는 `I am a student`가 들어가고, 디코더 쪽 정답 문장은 `moi suis étudiant </s>`로 표시됩니다.
...
- **디코더 입력**: 학습 단계에서 **정답 문장을 한 칸씩 밀어서** 넣습니다. 시작 토큰(`<s>`) 뒤에 정답 단어를 차례로 주고, **다음 단어를 맞히도록** 학습합니다.
- **디코더 출력**: 한 번에 전체가 아니라 **한 단어씩 순서대로** 생성합니다.

> 🟩 **표기 주의**: PDF p.17-p.20에는 예시가 여러 개 섞여 있습니다. **다이어그램의 선택 결과는 `moi suis étudiant </s>`** 입니다(확률표에서 `moi`=0.4, `suis`=0.6, `étudiant`=0.5, `</s>`=0.6이 각각 가장 높아 선택됨). 반면 p.18 본문의 `Je t'aime`은 디코더 입력 방식을 설명하는 **별도 예시**이고, 아래에 나오는 `Je: 0.80, Tu: 0.10, Il: 0.05, 기타: 0.05`도 확률 계산 방식을 보여주는 **또 다른 별도 예시**입니다. 셋을 같은 문장으로 혼동하지 않도록 구분해서 봐야 합니다.

> 🟩 **용어 보충 — Teacher Forcing**: 위처럼 학습할 때 디코더에 **직전의 정답 단어**를 넣어 다음 단어를 맞히게 하는 방식을 **Teacher Forcing(교사 강요)** 이라고 부릅니다. 정답이 없는 **추론 단계에서는** 디코더가 **직전에 스스로 생성한 단어**를 다음 입력으로 사용합니다.

🟦 (별도 예시) 디코더는 각 위치에서 **가능한 여러 단어 후보의 확률**을 계산하고, 그 **확률의 합은 1** 이 됩니다.
...
🟦 **디코더 입력과 디코더 출력**: 정답 문장이 `I LOVE YOU`라면, PDF는 그 앞부분을 예로 들어 디코더 입력을 `<sos> I LOVE`, 디코더 출력을 `I LOVE YOU <eos>`라고 설명합니다.

```text
디코더 입력 : <sos> I LOVE          (PDF 예시: 앞부분)
디코더 출력 : I LOVE YOU <eos>
```

> 🟩 **전체 시퀀스로 보면**: 위 예시는 앞부분만 보여준 것이고, 학습에 쓰는 **전체 입력·출력 쌍**을 나란히 두면 다음과 같습니다. 이렇게 둬야 마지막 `YOU`가 `<eos>`를 맞히는 것까지 포함됩니다.
...
즉 디코더는 **같은 위치끼리 비교해 다음 단어를 맞히도록** 학습합니다.
...
> 🟩 PDF p.24의 정수 인코딩 예시는 `I=1`, `LOVE=2`, `YOU=3`, `<sos>=4`, `<eos>=5`입니다. 아래 실습에서는 **패딩 결과까지 함께 보려고 `<pad>=0`을 추가**했습니다(PDF 원문에 있는 값이 아니라 패딩 데모를 위한 실습용 추가입니다). 그래서 원-핫 벡터 길이는 PDF 예시의 5가 아니라 `vocab_size=6`입니다.

```python
import numpy as np

# PDF 예시(I=1, LOVE=2, YOU=3, <sos>=4, <eos>=5)에 패딩용 <pad>=0을 추가한 실습용 vocab
...
> 🟩 **해석**: `디코더 입력`은 정답 앞에 `<sos>`를 붙인 것, `디코더 타깃`은 정답 뒤에 `<eos>`를 붙인 것입니다. 둘을 나란히 보면 "입력의 각 위치가 타깃의 같은 위치 단어를 맞혀야 한다"(`<sos>→I`, `I→LOVE`, `LOVE→YOU`, `YOU→<eos>`)는 걸 확인할 수 있습니다. 패딩은 문장 길이를 맞춰 배치 학습을 가능하게 하고, 원-핫 인코딩은 정수 인덱스를 신경망이 다루기 쉬운 벡터로 바꿉니다. (PDF는 "원-핫 인코딩을 **적용할 수 있다**"고 설명합니다 — 뒤에 나오듯 정답을 원-핫 대신 **정수 인덱스**로 두고 `sparse_categorical_crossentropy`를 쓰는 방식도 흔히 씁니다. 즉 원-핫이 유일한 방법은 아닙니다.)
...
🟩 **참고용 Keras 구조 코드(미실행, 구조 이해용 partial snippet)** — PDF가 글로 설명한 인코더/디코더 LSTM 연결 구조를 보여주기 위한 예시입니다. 실제로 학습을 돌리려면 `encoder_input_data`, `decoder_input_data`, `decoder_target_data`의 shape를 맞춰 따로 준비해야 합니다. TensorFlow/Keras가 설치되어 있지 않아 **실행하지 않았고, 출력값도 제시하지 않습니다.**
...
> 🟩 **참고**: 실제 어휘 수가 큰 번역 모델에서는 원-핫 벡터가 매우 커지므로, 정답을 **정수 인덱스**로 두고 `sparse_categorical_crossentropy`를 쓰는 구현이 흔합니다(이 부분은 PDF 밖 실무 보충입니다). 어떤 형태를 쓰든 "매 시점 단어 하나를 고르는 다중 분류"라는 본질은 같습니다.
>
...
5. **정수 인코딩 → 패딩 → 원-핫**: 문자를 숫자로, 길이를 맞추고, 신경망 입력용 벡터로 변환하는 순서(원-핫은 여러 선택지 중 하나).
...
번역 기술 발전
  ...
  - 🟩 GNMT(2016)는 LSTM enc-dec + attention → 이후 Transformer가 attention 더 중심
...
실제 구현 흐름 (Keras/LSTM)
  - SOS/EOS 토큰 → 정수 인코딩 → 패딩 → 원-핫 인코딩(또는 정수 인덱스)
