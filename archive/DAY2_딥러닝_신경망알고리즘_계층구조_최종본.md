...
> 이 코드는 **학습이 아니라 운에 기대는 무작위 탐색**입니다. 조건을 만족하는 가중치를 우연히 뽑을 때까지 반복하므로, 운이 나쁘면 오래 걸릴 수 있습니다. 실습할 때는 시도 횟수에 상한을 두는 것이 안전합니다(아래는 보완 예시).
>
> ```python
...
model = nn.Linear(2, 1)                            # 단일 퍼셉트론(선형 계층)
...
순전파가 예측을 만드는 과정이라면, 역전파는 **오차의 원인을 뒤에서 앞으로 추적**하는 과정입니다. 역전파는 손실값을 각 가중치에 대해 **미분**하며, 이 값이 **Gradient(기울기)** 입니다.
...
거의 모든 PyTorch 학습 코드는 이 구조를 따릅니다.
